...
- 自动同步新版 Codex 的 bundled marketplace 快照
- 自动把官方 bundled 插件写入 `~/.codex/config.toml` 并设为启用
- 开启 `goals` 目标模式
- 开启运行时防止系统休眠
- 开启接通电源时远程控制保持唤醒
- 尽量保持本地线程视图优先
...
当前版本已经把注入脚本更新到 `codexplus-v4`，并兼容 `添加到 Codex` / `Add to Codex`。

从 Codex `26.608.x` 开始，官方包里的 Node 路径从：
...
新版 Codex 更新后，有些官方 bundled 插件还在官方包里，但用户目录里可能只剩一部分旧快照。只复制插件 cache 不够，Codex 还会看本地 bundled marketplace 和用户配置里的启用状态。

CodexPlus 现在会扫描官方包里的：
...
同时会把官方自带的 marketplace manifest 同步到：

```text
~/.codex/.tmp/bundled-marketplaces/openai-bundled/
```

并在 `~/.codex/config.toml` 里补齐当前 Codex.app 内置的官方插件：
...
这样 Chrome、Computer Use、Browser、LaTeX、Sites 这类官方 bundled 插件重新加载后都能被 Codex 找到。

如果只想手动刷新 bundled 插件，不启动或重启 Codex，可以在项目目录里执行：

```bash
node unlocker.mjs sync-bundled-plugins
```

如果想看当前缺不缺插件，可以执行：

```bash
node unlocker.mjs status
```

正常情况下这几个数字应该为 `0`：

```text
bundledMarketplaceMissingCount
bundledPluginConfigMissingCount
```

### 关于 API 模式和账号功能

这次看了一遍 CodexPlusPlus 的实现后，结论比较明确：它后来的“官方账号 + API Key 混合模式”不是单纯把 API 登录伪装成账号登录。

它的关键思路是保留 Codex 的 ChatGPT 登录态，让账号侧能力继续走官方登录，再把模型请求切到自定义接口：
...
也就是说，真正接近“API Key 也享受账号登录待遇”的路线，是保留账号登录态，再做模型请求转发；如果完全纯 API 模式，某些由官方账号、地区、组织策略控制的能力，CodexPlus 只能打开本地入口和本地插件发现，不能凭空伪造后端授权。

如果确实要回到旧的“强制调试端口重启”模式，可以手动设置：

```bash
CODEXPLUS_FORCE_DEBUG_RELAUNCH=1 open -a /Applications/CodexPlus.app
```

这个模式可能影响新版 Codex 启动，平时不建议用。
